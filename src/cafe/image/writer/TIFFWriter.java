...
import cafe.image.compression.ccitt.G32DEncoder;
import cafe.image.compression.ccitt.G42DEncoder;
...
public class TIFFWriter extends ImageWriter implements Updatable<Integer> {
	 ...
	 * See for example {@link cafe.image.tiff.TIFFTweaker#insertPage}
	 * 
	...
	private void writeIndexed(int[] pixels, int imageWidth, int imageHeight, Compression compression) throws Exception {		
		...
		if(colorInfo[0]>0x08) {
			if(getImageParam().isApplyDither())
				colorInfo = IMGUtils.reduceColorsFloydSteinberg(pixels, imageWidth, imageHeight, 8, newPixels, colorPalette);
			else
